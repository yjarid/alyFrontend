...
import { withStyles, MenuItem, FormHelperText } from "@material-ui/core"
...
const MyTextFieldPre = props => {
  ...
  return <TextField {...field} className={classes.root} classes={classes} disabled={disabled} type={type} label={label} helperText={errorText} autoComplete="off" error={errorText ? true : false} margin="normal" variant="outlined" fullWidth />
}
...
export const MyTextArea = props => {
  ...
  const [field, meta] = useField(props)
  const errorText = meta.error && meta.touched ? meta.error : ""
  return <TextField {...field} multiline rows={rows} label={label} helperText={errorText} error={errorText ? true : false} margin="normal" variant="outlined" fullWidth />
}
...
const MaterialUISelectField = ({ errorString, children, value, name, label, onChange, onBlur }) => {
  return (
