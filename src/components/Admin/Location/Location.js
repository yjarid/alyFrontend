          ...
          <Fragment>
            <div className={styles.busList}>
              {citiesName.map((city, i) => (
                <p key={i} className={styles.busItem} onClick={() => setBusCity(city)}>
                  {city}
                </p>
              ))}
            </div>
            ...
            <div className={styles.busList}>
              {neighberhood.map((loc, i) => (
                <p key={i} className={styles.busItem} onClick={() => setBusIndex(i)}>
                  {loc.name}
                </p>
              ))}
            </div>
            <div>{(index != null || type === "create") && <PreviewLocation location={neighberhood[index]} cities={citiesName} type={type} />}</div>
          </Fragment>
        )}
      </div>
    </Fragment>
